...
public:
    ...
    void gpu_bench(GL &gpu_layout, FS bench_setup, FC bench_compute) {
        ...
        hipMemcpy(gpu_result.data(), gpu_layout.device_result, gpu_result.size() * sizeof(hostTt), hipMemcpyDeviceToHost);
...
struct TestGPULayout {
    ...
    TestGPULayout() {}
    ~TestGPULayout() {
        if (device_array) hipFree(device_array);
        if (device_result) hipFree(device_result);
    }
};
...
void test_gpu_setup(vector<hostTs> &host_array, TestGPULayout<deviceTs, deviceTt> &layout)
{
    ...
    hipMalloc((void**)&layout.device_array, host_array.size() * sizeof(deviceTs));
    hipMalloc((void**)&layout.device_result, layout.n * sizeof(deviceTt));

    hipMemcpy(layout.device_array, host_array.data(), host_array.size() * sizeof(deviceTs), hipMemcpyHostToDevice);
}
