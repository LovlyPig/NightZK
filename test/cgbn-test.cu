...
struct PointAddGPULayout {
    ...
    PointAddGPULayout() { 
        hipMalloc(&warp_result, 32 * sizeof(g1_t));
    }

    ~PointAddGPULayout() {
        if (points) hipFree(points);
...
void cuda_pointadd_setup(vector<libff::G1<ppT>> points, PointAddGPULayout &gpu_layout)
{
    ...
    hipMalloc(&gpu_layout.points, gpu_layout.n * sizeof(g1_t::affine_t));
    ...
    hipMemcpy(gpu_layout.points, temp.data(), gpu_layout.n * sizeof(g1_t::affine_t), hipMemcpyHostToDevice);
}
...
void cuda_pointadd_compute(PointAddGPULayout &gpu_layout, libff::G1<ppT> &result)
{
    ...
    hipMemcpy(&result, gpu_layout.warp_result, sizeof(libff::G1<ppT>), hipMemcpyDeviceToHost);
}
...
//     hipMemcpy(&result, gpu_layout.result, sizeof(libff::Fr<ppT>), hipMemcpyDeviceToHost);
// }
