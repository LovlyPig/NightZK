...
public:
    ...
    {
        hipHostRegister(x.data(), x.size() * sizeof(libff::Fr<ppT>), hipHostRegisterDefault);
...
struct SpMVGPULayout {
    ...
    {
        hipMalloc(&d_x, cols * sizeof(fr_t));
        for (int i = 0; i < INSTANCE; i++) hipMalloc(&d_y[i], rows * sizeof(fr_t));
    }

    ~SpMVGPULayout() { hipFree(d_x); for (int i = 0; i < INSTANCE; i++) hipFree(d_y[i]); }
};

void cuda_spmv_setup(SparseMatrix<libff::Fr<ppT>> &mat, vector<libff::Fr<ppT>> &x, SpMVGPULayout &gpu_layout)
{
    hipMemcpy(gpu_layout.d_x, x.data(), gpu_layout.cols * sizeof(fr_t), hipMemcpyHostToDevice);
}
...
void cuda_spmv_load(SpMVGPULayout &gpu_layout, vector<libff::Fr<ppT>> &gpu_result)
{
    ...
    hipMemcpy(gpu_result.data(), gpu_layout.d_y[INSTANCE - 1], gpu_layout.rows * sizeof(fr_t), hipMemcpyDeviceToHost);
}
