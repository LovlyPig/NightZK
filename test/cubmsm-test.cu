...
struct MSMGPULayout {
    ...
    {
        hipMalloc(&bucket_start, num_windows * num_buckets * sizeof(uint32_t));
        hipMalloc(&bucket_size, num_windows * num_buckets * sizeof(uint32_t));
    }

    ~MSMGPULayout()
    {
        if (scalars) hipFree(scalars);
        if (indices) hipFree(indices);

        hipFree(bucket_start);
        hipFree(bucket_size);
    }
};
...
void cuda_msm_setup(vector<libff::Fr<ppT>> scalars, MSMGPULayout gpu_layout)
{
    ...
    hipMalloc(&gpu_layout.scalars, gpu_layout.n * sizeof(fr_t));
    hipMemcpy(gpu_layout.scalars, scalars.data(), gpu_layout.n * sizeof(fr_t), hipMemcpyHostToDevice);

    hipMalloc(&gpu_layout.indices, gpu_layout.num_windows * gpu_layout.n * sizeof(uint32_t));
}
